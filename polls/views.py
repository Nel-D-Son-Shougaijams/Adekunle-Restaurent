...
def orders(request):
    ...
    if request.user.is_authenticated:
        ...
        if Cart.objects.filter(user=request.user,ordered=True).exists():
        ...
        no = False
        details = {}
        orderitem = Order.objects.get(carts=carts)
        details[f"Cart{b}"] = {"products":carts,"current":orderitem.current,"details":orderitem.details,"created_at":orderitem.created_at}
